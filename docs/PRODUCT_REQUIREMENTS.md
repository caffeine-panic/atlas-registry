...
- 生产连接默认只读；写入必须以精确连接名开启 5–60 分钟的会话级窗口，断开、重启或到期立即失效，所有 mutation 在 dispatch 前统一复检。
- 支持当前目标与另一已连接同协议环境的单资源精确比较；提升前刷新来源与目标并复用 Safe Change Center，协议不兼容、缺失、二进制、超限、无权限、取消与过期结果必须明确呈现。
- 连接创建/探测、工作区浏览/搜索与资源查看的完整只读旅程支持 English / 简体中文；用户语言独立持久化，不支持的系统语言回退英文，协议标识与服务端内容不翻译。
- 现有资源更新进入 Safe Change Center：有界结构化 diff、权威预检、协议并发令牌、显式确认、单次条件提交、权威回读和脱敏收据构成不可跳步的完整路径。
- etcd 使用 mod revision，ZooKeeper 使用 version，Nacos 使用 MD5/CAS（可用时）避免静默覆盖。
- 递归删除、批量变更和生产环境写入必须二次确认。
- 编辑前后快照进入本地脱敏审计记录。
- applied、conflict、远端结果未知和审计未完成必须作为不同终态呈现；收据不得包含 value、密码、token 或 endpoint 参数。
