...
test("registry errors are classified exclusively by their structured code", () => {
  ...
  assert.equal(
    errors.isRegistryError(auditIncomplete, "auditIncomplete"),
    true,
  );
  assert.equal(
    errors.isRegistryError(auditIncomplete, "outcomeUnknown"),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    errors.isRegistryError({ code: "auditIncomplete" }, "auditIncomplete"),
    false,
  );
  assert.equal(
    errors.isRegistryError(
...
test("registry error messages remain safe for unknown rejection values", () => {
  assert.equal(
    errors.registryErrorMessage({ message: "remote failed" }),
    "remote failed",
  );
...
test("mutation failures select recovery behavior from structured codes", () => {
  ...
  assert.equal(
    errors.mutationFailureRecovery(failure("outcomeUnknown")),
    "unknownOutcome",
  );
  assert.equal(
    errors.mutationFailureRecovery(failure("auditIncomplete")),
    "unknownOutcome",
  );
