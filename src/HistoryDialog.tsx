    ...
    <div className="dialog-backdrop" onMouseDown={onClose}>
      ...
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">LOCAL AUDIT HISTORY</span>
            <h2>变更历史</h2>
          </div>
        ...
        <div className="history-list">
          {!loading && items.length === 0 && (
            <div className="empty compact">
              <b>暂无本地变更记录</b>
              <span>成功、失败和结果未知的写入会出现在这里。</span>
            </div>
          )}
          {items.map((item, index) => (
            <article
              className={`history-item ${item.kind}`}
              key={`${item.operationId}-${item.kind}-${item.timestampMs}-${index}`}
            >
              ...
              <div className="history-target">
                <span>
                  {profileNames.get(item.connectionId) ?? item.connectionId}
                </span>
                <b>
                  {item.address
              ...
              <div className="history-details">
                {item.expectedVersion && (
                  <span>期望版本 {item.expectedVersion}</span>
                )}
                {item.nativeTarget && <span>{item.nativeTarget}</span>}
                {item.consistency && (
                  <span>
                    {item.consistency === "atomic" ? "原子条件" : "检查后变更"}
                  </span>
                )}
        ...
        <div className="dialog-actions split-actions">
          <span className="form-note history-note">
            日志只展示版本、大小、编码和 SHA-256 摘要。
          </span>
...
function snapshotText(snapshot: ResourceSnapshot | undefined, label: string) {
  ...
  return (
    <span>
      {label} {snapshot.sizeBytes.toLocaleString()} B ·{" "}
      {snapshot.encoding.toUpperCase()} · {snapshot.sha256.slice(0, 12)}…
    </span>
  );
}
...
function addressText(address: ResourceAddress) {
  switch (address.type) {
    case "root":
      return "/";
    case "etcd":
      return decodeEtcdKey(address.keyBase64);
    case "etcdPrefix":
      return `prefix base64:${address.prefixBase64}`;
    case "zookeeper":
      return address.path;
    case "nacosConfig":
