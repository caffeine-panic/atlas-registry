...
当前公开安装包尚无 Apple Developer ID/公证或 Windows Authenticode 签名。macOS 产物仅有 ad-hoc 签名，macOS/Windows 可能提示安全警告或阻止安装。Tauri 更新签名只验证更新资产，不能替代操作系统的发行者信任认证。请只使用官方 Release 资产，并在安装前阅读对应发布说明。
...
- 连接、浏览/搜索与资源查看旅程支持 English / 简体中文；首次启动对简体中文系统语言使用中文，其余语言回退英文，显式选择独立保存在本机且不进入连接配置或凭据库
...
| Nacos     | 配置历史；Nacos 3 AI Registry 只读浏览（按服务器能力展示 MCP、Prompt、Skill、A2A）；namespace、service 和 instance 管理；Nacos 2.x / 3.x |
...
- etcd 用户名/密码与 TLS
- etcd 单跳受管 SSH 隧道：固定 SHA-256 主机密钥指纹，支持密码或私钥认证
- ZooKeeper Digest 认证与 TLS
- Nacos 用户名/密码、自定义认证上下文或阿里云 MSE AccessKey（AK/SK）

AI Registry 浏览器按 Nacos 3.2.3 Admin API 契约验证，并逐类探测能力；较早的 3.x 服务端只显示真实存在的资源族。该入口只展示经过限长和脱敏的标量元数据，不读取 Prompt 模板、MCP 工具定义或 Skill 包，也不提供创建、更新、发布和删除操作。

连接配置保存在本机应用目录，注册中心密钥与 SSH 密码/私钥口令分别保存在系统凭据库。临时密钥只用于当前连接，不写入连接配置。SSH 主机密钥与固定指纹不一致时会直接拒绝连接。
...
可在 **设置 → 界面语言** 中切换 English / 简体中文。协议名、资源标识、元数据名和服务端返回的 value 永远不会被翻译或改写。
