...
fn audit_log_persists_started_and_applied_events_without_resource_values() {
    tauri::async_runtime::block_on(async {
        ...
        log.record_production_unlocked_in(&directory, "connection-1", "operation-4", 900)
            .await
            .expect("production unlock event should be written");
        log.record_production_locked_in(&directory, "connection-1", "operation-5")
            .await
            .expect("production lock event should be written");
        ...
        assert!(content.contains("productionUnlocked"));
        assert!(content.contains("productionLocked"));
        assert!(content.contains("\"duration_seconds\":900"));
        ...
        assert!(!content.contains("https://secret-endpoint.example"));
        assert!(!content.contains("production-profile-name"));
        ...
        assert_eq!(first_page.items[0].kind, AuditHistoryKind::ProductionLocked);
        assert_eq!(
            first_page.items[1].kind,
            AuditHistoryKind::ProductionUnlocked
        );
        assert_eq!(first_page.items[1].duration_seconds, Some(900));
        ...
        assert_eq!(second_page.items[0].kind, AuditHistoryKind::Applied);
        assert_eq!(
            second_page.items[0].native_operation,
            Some(NativeAuditOperation::EtcdLeaseKeepAlive)
        );
        assert_eq!(
            second_page.items[0].native_target.as_deref(),
            Some("lease:9223372036854775807")
        );
        assert_eq!(second_page.items[1].kind, AuditHistoryKind::Started);
        ...
        let third_page = log
            ...
            .await
            .expect("older audit events should load by cursor");
        assert_eq!(third_page.items.len(), 2);
        assert_eq!(third_page.items[0].kind, AuditHistoryKind::OutcomeUnknown);
        assert_eq!(third_page.items[1].kind, AuditHistoryKind::Applied);
        ...
        let fourth_page = log
            .load_recent_in(
                &directory,
                Some("connection-1"),
                third_page.next_cursor.clone(),
                2,
            )
            .await
            .expect("oldest audit event should load by cursor");
        assert_eq!(fourth_page.items.len(), 1);
        assert_eq!(fourth_page.items[0].kind, AuditHistoryKind::Started);
        assert!(fourth_page.next_cursor.is_none());
